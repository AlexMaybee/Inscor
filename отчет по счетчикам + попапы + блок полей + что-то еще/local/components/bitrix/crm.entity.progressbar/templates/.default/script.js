...
/*Блок всех счетчиков и progressbar вверху, строка № 115*/
function blockNumbersFields() {
    $('#entity_progress_container .crm-entity-section-status-step').css({'pointer-events':'none','opacity':'0.3'}); // progressbar
    $('div[data-cid="UF_CRM_1532617063"],div[data-cid="UF_CRM_1533559778"]').css({'pointer-events':'none','opacity':'0.3'}); // счетчики "Актуализация"
    $('div[data-cid="UF_CRM_1532617099"],div[data-cid="UF_CRM_1533559811"]').css({'pointer-events':'none','opacity':'0.3'}); // счетчики "Ожидание"
    $('div[data-cid="UF_CRM_1532617079"],div[data-cid="UF_CRM_1533559839"]').css({'pointer-events':'none','opacity':'0.3'}); // счетчики "Тендер"
    $('div[data-cid="UF_CRM_1533557380681"],div[data-cid="UF_CRM_1533559867"]').css({'pointer-events':'none','opacity':'0.3'}); // счетчики "Доп. Вопросы"
   // $('div[data-cid="UF_CRM_1536058832"]').css({'pointer-events':'none','opacity':'0.3'}); // счетчики "ID задачи на стадии Ожидание"
};


//Popup handler
function Popuphandler(dealID) {
	if(dealID > 0){
        data = {'ID':dealID};
...
//Popup причины ПРОВАЛА
function showLosePopup(dealID,reasons,newDealDate) {
    var Dialog = new BX.CDialog({
        title: "Причины провала сделки и создание новой сделки",
        head: 'Укажите причины провала и дату новой сделки',
        content: '<form method="POST" style="overflow:hidden;" action="" id="formLoseUpdate"><div class="crm-entity-widget-content-block crm-entity-widget-content-block-field-custom-text" data-cid="UF_CRM_1533715533"><div class="crm-entity-widget-content-block-draggable-btn-container"><div class="crm-entity-widget-content-block-draggable-btn"></div></div><div class="crm-entity-widget-content-block-title"><span class="crm-entity-widget-content-block-title-text">Причины провала</span></div><div class="crm-entity-widget-content-block-inner"><span class="fields string field-wrap"><span class="fields string field-item"><textarea cols="20" rows="4" class="fields string" name="UF_CRM_1533715533" tabindex="0"></textarea></span></span></div><div></div></div><div class="crm-entity-widget-content-block crm-entity-widget-content-block-field-custom-date crm-entity-widget-content-block-edit" data-cid="UF_CRM_1533826216"><div class="crm-entity-widget-content-block-title"><span class="crm-entity-widget-content-block-title-text">Выберете дату следующей сделки</span></div><div class="crm-entity-widget-content-block-inner"><span class="fields date field-wrap"><span class="fields date field-item"><input onclick="BX.calendar({node: this, field: this, bTime: false, bSetFocus: false})" name="UF_CRM_1533826216" type="text" tabindex="0" value=""><i class="fields date icon" onclick="BX.calendar({node: this.previousSibling, field: this.previousSibling, bTime: false, bSetFocus: false});"></i></span></span></div><div></div></div></form>',
        icon: 'head-block',
        resizable: true,
        draggable: true,
        height: '310',
        width: '600',
        buttons: ['<input type="submit" value="Сохранить" name="popupSubmit" id="sentReasonLose"/>',BX.CDialog.btnClose]
    });
    $('#sentReasonLose').click(function () {
        validateAndSentLoseReason(dealID);
    });
    //console.log(ID);
    Dialog.Show(); //запуск popup
    $('#formLoseUpdate textarea').val(reasons);
    $('#formLoseUpdate input[name="UF_CRM_1533826216"]').val(newDealDate);
    $('#bx-admin-prefix .bx-core-adm-dialog-head-icons .bx-core-adm-icon-close').css('display','none');
    $('#bx-admin-prefix .bx-core-adm-dialog-buttons #close').css('visibility','hidden');
}

/*Popup причины СЛИВА*/
function showWaistedPopup(dealID,reasons) {
    var Dialog = new BX.CDialog({
        title: "Причина слива сделки и создание новой сделки",
        head: 'Выберите причину слива и дату новой сделки',
        content: '<form method="POST" style="overflow:hidden;" action="" id="formLoseUpdate"><div class="crm-entity-widget-content-block crm-entity-widget-content-block-field-custom-select" data-cid="UF_CRM_1533826350"><div class="crm-entity-widget-content-block-draggable-btn-container"><div class="crm-entity-widget-content-block-draggable-btn"></div></div><div class="crm-entity-widget-content-block-title"><span class="crm-entity-widget-content-block-title-text">Причина слива</span></div><div class="crm-entity-widget-content-block-inner"><span class="fields enumeration field-wrap"><input type="hidden" name="UF_CRM_1533826350" value="" id="UF_CRM_1533826350_default"><span class="fields enumeration enumeration-select field-item"><select name="UF_CRM_1533826350" tabindex="0"><option value="">нет</option><option value="4829">Брокер</option><option value="4830">Продали ТС</option><option value="4831">Закрылась компания</option><option value="4832">Не интересен</option><option value="4833">только ОСАГО</option></select></span></span></div><div></div></div></form>',
        icon: 'head-block',
        resizable: true,
        draggable: true,
        height: '310',
        width: '600',
        buttons: ['<input type="submit" value="Сохранить" name="popupSubmit" id="sentReasonWaisted"/>',BX.CDialog.btnClose]
    });
    $('#sentReasonWaisted').click(function () {
        validateAndSentWaistedReason(dealID);
    });
    //console.log(ID);
    Dialog.Show(); //запуск popup
   $('#formLoseUpdate input[name="UF_CRM_1533826350"]').val(reasons);
    $('#bx-admin-prefix .bx-core-adm-dialog-head-icons .bx-core-adm-icon-close').css('display','none');
    $('#bx-admin-prefix .bx-core-adm-dialog-buttons #close').css('visibility','hidden');
}

/*Popup ЗАСТРАХОВАН*/
function showInsuredPopup(dealID,newDealDate) {
    var Dialog = new BX.CDialog({
        title: "Дата создания новой сделки",
        head: 'Укажите дату для создания новой сделки',
        content: '<form method="POST" style="overflow:hidden;" action="" id="formLoseUpdate"><div class="crm-entity-widget-content-block crm-entity-widget-content-block-field-custom-date crm-entity-widget-content-block-edit" data-cid="UF_CRM_1533826216"><div class="crm-entity-widget-content-block-title"><span class="crm-entity-widget-content-block-title-text">Выберете дату следующей сделки</span></div><div class="crm-entity-widget-content-block-inner"><span class="fields date field-wrap"><span class="fields date field-item"><input onclick="BX.calendar({node: this, field: this, bTime: false, bSetFocus: false})" name="UF_CRM_1533826216" type="text" tabindex="0" value=""><i class="fields date icon" onclick="BX.calendar({node: this.previousSibling, field: this.previousSibling, bTime: false, bSetFocus: false});"></i></span></span></div></div></form>',
        icon: 'head-block',
        resizable: true,
        draggable: true,
        height: '250',
        width: '500',
        buttons: ['<input type="submit" value="Сохранить" name="popupSubmit" id="sentNewDealDate"/>',BX.CDialog.btnClose]
    });
    $('#sentNewDealDate').click(function () {
        validateAndSentInsuredNewDate(dealID);
    });
    //console.log(ID);
    Dialog.Show(); //запуск popup
...
//Валидация popup При Проигрыше Сделки
function validateAndSentLoseReason(dealID) {
	var reasons = $('#formLoseUpdate textarea').val();
	var new_deal_data = $('#formLoseUpdate input[name="UF_CRM_1533826216"]').val();
    var regEx = /^([0]?[1-9]|[1|2][0-9]|[3][0|1])[.]([0]?[1-9]|[1][0-2])[.]([0-9]{4})$/;

    $('#formLoseUpdate textarea').css('border-color','#c4c7cc')
    $('#formLoseUpdate input[name="UF_CRM_1533826216"]').css('border-color','#c4c7cc');

    $('#formLoseUpdate .MyPopupError').remove();
	if(reasons.length < 20 || checkValidDate(new_deal_data) == false || !new_deal_data.match(regEx)) {
    	if(reasons.length < 20){
            $('#formLoseUpdate textarea').css('border-color','red');
            $('#formLoseUpdate div[data-cid="UF_CRM_1533715533"]').after('<div class="MyPopupError" style="color:red;font-weight:600;margin:10px 0;">В пояснении должно быть не менее 20 символов!</div>');
        }
        if(checkValidDate(new_deal_data) == false || !new_deal_data.match(regEx)){
            $('#formLoseUpdate input[name="UF_CRM_1533826216"]').css('border-color','red');
            if(checkValidDate(new_deal_data) == false) $('#formLoseUpdate div[data-cid="UF_CRM_1533826216"]').after('<div class="MyPopupError" style="color:red;font-weight:600;margin:10px 0;">Дата следующей сделки не может быть прошедшей!</div>');
            if(!new_deal_data.match(regEx)) $('#formLoseUpdate div[data-cid="UF_CRM_1533826216"]').after('<div class="MyPopupError" style="color:red;font-weight:600;margin:10px 0;">Выберете корректный формат даты следующей сделки!</div>');
        }
	}
    else {
		var result = {'DATA':reasons,'ID':dealID,'DEAL_DATE':new_deal_data,'POPUP':'LOSE'};
        ajaxRequest(result);
	}
}

//Валидация popup При Сливе Сделки
function validateAndSentWaistedReason(dealID) {
    var reasons = $('#formLoseUpdate select[name="UF_CRM_1533826350"]').val();

    $('#formLoseUpdate select[name="UF_CRM_1533826350"]').css('border-color','#c4c7cc')

    $('#formLoseUpdate .MyPopupError').remove();
    if(reasons.length < 2) {
		$('#formLoseUpdate select[name="UF_CRM_1533826350"]').css('border-color','red');
		$('#formLoseUpdate div[data-cid="UF_CRM_1533826350"]').after('<div class="MyPopupError" style="color:red;font-weight:600;margin:10px 0;">Выберите причину слива сделки!</div>');
    }
    else {
        var result = {'DATA':reasons,'ID':dealID,'POPUP':'WAISTED'};
        //ajaxRequest(result);
        ajaxWaisted(result);
    }
}

//Валидация popup ЗАСТРАХОВАН!!!
function validateAndSentInsuredNewDate(dealID) {
    var new_deal_data = $('#formLoseUpdate input[name="UF_CRM_1533826216"]').val();
    var regEx = /^([0]?[1-9]|[1|2][0-9]|[3][0|1])[.]([0]?[1-9]|[1][0-2])[.]([0-9]{4})$/;
    ...
    if(checkValidDate(new_deal_data) == false || !new_deal_data.match(regEx)) {
        ...
        var result = {'ID':dealID,'DEAL_DATE':new_deal_data,'POPUP':'INSURED'};
        ajaxRequest(result);
    }
}

//ajax запрос на сохранение
function ajaxRequest(dataObj) {
    BX.ajax({
        method: "POST",
        ...
        onsuccess: function (data) {
			...
			//Так чтобы при просмотре уже проигранной сделки не показывался popup
			if(data.result.STAGE_ID == 'LOSE' && (data.result.UF_CRM_1533715533 == null && data.result.UF_CRM_1533826216 == '')) showLosePopup(dataObj.ID,data.result.UF_CRM_1533715533,data.result.UF_CRM_1533826216);

			if(data.result.STAGE_ID == 3 && data.result.UF_CRM_1533826350 == null) showWaistedPopup(dataObj.ID,data.result.UF_CRM_1533826350);

			if(data.result.STAGE_ID == 'WON' && data.result.UF_CRM_1533826216 == '') showInsuredPopup(dataObj.ID,data.result.UF_CRM_1533826216);

			if(data.result == true) {
                $('#bx-admin-prefix .bx-core-adm-dialog-buttons #close').css('visibility','visible');
                $('#sentReasonLose').css('visibility','hidden');
                $('#sentNewDealDate').css('visibility','hidden');
                $('.bx-core-adm-dialog-content-wrap-inner div').remove();
                $('.bx-core-adm-dialog-content-wrap-inner').append('<div class="MyPopupError" style="color:green;font-weight:600;font-size:20px;margin-top:40px;text-align:center;">Изменения сохранены и создана новая сделка!<br>Для просмотра перезагрузите страницу!<br>Окно закроется через 2 секунды автоматически</div>');
                setTimeout(closePopupAfterSuccess, 2000);
			}
        }
    });
}

//отдельный запрос для стадии "Слит"
function ajaxWaisted(dataObj) {
    BX.ajax({
        method: "POST",
        url: '/local/ajax/ajax_handler.php',
        data: dataObj,
        ...
        onsuccess: function (data) {
            ...
            if(data.result == true) {
                $('#bx-admin-prefix .bx-core-adm-dialog-buttons #close').css('visibility','visible');
                $('#sentReasonWaisted').css('visibility','hidden');
                $('.bx-core-adm-dialog-content-wrap-inner div').remove();
                $('.bx-core-adm-dialog-content-wrap-inner').append('<div class="MyPopupError" style="color:green;font-weight:600;font-size:20px;margin-top:40px;text-align:center;">Изменения сохранены в текущей карточке сделки!<br>Для просмотра перезагрузите страницу и откройте снова!<br>Всплывающее окно закроется через 2 секунды автоматически</div>');
                setTimeout(closePopupAfterSuccess, 2000);
            }
        }
    });
...
//проверяет время для валидации (чтобы не могли выбрать прошедшее дату/время)
function checkValidDate(str) {
    if(new Date(str.replace(/(\d+)[.](\d+)[.](\d+)/, '$2/$1/$3')) < (new Date())) return false
    else return true;
}

/*Автозакрытие popup через установленное время при вызове метода*/
function closePopupAfterSuccess() {
    $('#bx-admin-prefix .bx-core-adm-dialog-buttons #close').click();
}