...
<script>
    ...
    //Кнопка изменить
    BX.bindDelegate(
        document.body, 'click', {className: 'crm-entity-widget-hide-btn' },
        function(e){
            if(ent_id == 2){
                blockNumbersFields();
            }
    ...
    BX.bindDelegate(
        document.body, 'click', {className: 'ui-btn-success' },
        function(e){
            if(ent_id == 2){
                blockNumbersFields();
                setTimeout("Popuphandler(<?=$entityID?>)", 1000);
            }
        }
    );

    //Клик на кнопку Отменить изменения
    BX.bindDelegate(
        document.body, 'click', {className: 'ui-btn-link' },
        function(e){
            if(ent_id == 2){
                blockNumbersFields();
            }
